...
st.dataframe(df_racer.style
             .highlight_max(color='lightgreen', axis=0, subset=['Speed'])
...
st.header('Racer Speed does not seem to correlate with the number of races a character has won.')
...
with right_column_1:
    ...
    df_winniest_racers = df_racer[['Character', 'Times First Place', 'Total Races']]
    df_winniest_racers['Win Percent'] = df_winniest_racers['Times First Place']/df_winniest_racers['Total Races'] * 100
    df_winniest_racers = df_winniest_racers[['Character', 'Win Percent']].sort_values('Win Percent', ascending=False).iloc[0:x]
    st.dataframe(df_winniest_racers)
